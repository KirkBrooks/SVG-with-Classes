...
Function load($source : Variant; $validate : Boolean; $schema : Text)->$this : cs:C1710.xml
	...
	Case of 
		...
		: (Value type:C1509($source)=Is text:K8:3) | (Value type:C1509($source)=Is BLOB:K8:12)  // Parse a given variable
			Case of 
				: (Count parameters:C259=1)
...
Function find($node : Text; $xpath : Text)->$references : Collection
	// Searches for one or more elements corresponding to an XPath
	// & returns a references collection if success.
	$references:=New collection:C1472()
	ARRAY TEXT:C222($nodes; 0)
	
	If (This:C1470._requiredParams(Count parameters:C259; 1))
		
		If (This:C1470.isReference($node)) & (Count parameters:C259>=2)
			$nodes{0}:=DOM Find XML element:C864($node; $xpath; $nodes)
			
		Else   // Start at the root, query string is the 1st parameter
...
Function previousSibling($node : Text; $name : Text)->$reference : Text
	...
	//MARK:-  Element value getters & setters
	...
	If (This:C1470._requiredParams(Count parameters:C259; 1))
		...
		If (This:C1470._requiredRef($node))
			...
			If (This:C1470.success)
				...
				If (Length:C16($elementValue)=0)  // Try CDATA
...
Function getContent($keepStructure : Boolean)->$content : Blob
	...
	//MARK:-  Element attributes getters & setters
